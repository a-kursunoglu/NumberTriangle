...
public class NumberTriangle {
    ...
    public static NumberTriangle loadTriangle(String fname) throws IOException {
        ...
        while (line != null) {
            ...
            List<NumberTriangle> thisRow = new ArrayList<>();
            for (String part : parts) {
                thisRow.add(new NumberTriangle(Integer.parseInt(part)));
            }
            rows.add(thisRow);
            line = br.readLine();
        }
        ...
        for (int r = 0; r < rows.size() - 1; r++) {
            List<NumberTriangle> parentRow = rows.get(r);
            List<NumberTriangle> childRow  = rows.get(r + 1);
            for (int c = 0; c < parentRow.size(); c++) {
                NumberTriangle parent = parentRow.get(c);
                parent.setLeft(childRow.get(c));
                parent.setRight(childRow.get(c + 1));
            }
        }

        return rows.get(0).get(0); // return the top
    }
